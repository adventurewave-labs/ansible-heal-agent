...
> `/home/z/my-project/download/PRD-Ansible-Heal-Agent.docx`. The DOCX is the
...
The agent is LLM-first: GLM-4-Plus (via the `z-ai-web-dev-sdk` CLI) performs the
natural-language diagnosis and proposes a structured JSON patch. A deterministic
rule-based fallback guarantees the agent still converges when the LLM is unavailable,
rate-limited, or produces malformed output. Every action the agent takes — prompts,
responses, patches, commits, pipeline re-runs — is captured in a timestamped Markdown
transcript stored alongside the repo, giving human reviewers full visibility into the
agent's reasoning chain.
...
from a typical 15–40 minute human-driven window to under 60 seconds; on-call SREs
are paged only for novel failures the agent explicitly declines to handle. The MVP
is intentionally simulated (no real SSH, no real apt) so the design space can be
validated safely before any production rollout.
...
| Pipeline watcher       | Tails the mock runner's log file (or real Ansible log stream).               |
| Log scanner            | Extracts structured failure records from raw log lines + sidecar JSON.       |
| Diagnoser (LLM + FB)   | Calls GLM-4-Plus via z-ai CLI; falls back to rule-based on any error.       |
| Patcher                | Applies a single string-replace; validates YAML before writing.              |
| Committer              | Stages the touched file, creates a conventional commit.                     |
| Pipeline restarter     | Re-runs the pipeline; loop continues until green or iteration cap.           |
...
|-------|----------|-------------|
| FR-1  | MUST     | Agent detects pipeline non-zero exit within 5 seconds. |
| FR-2  | MUST     | Agent extracts structured failure records (type, host, message, playbook, task). |
| FR-3  | MUST     | Agent calls GLM-4-Plus via z-ai CLI for diagnosis; falls back to rule-based on any error. |
| FR-4  | MUST     | Patcher applies a single string-replace operation to one of the allowed target files. |
| FR-5  | MUST     | Patcher validates the patched file is still valid YAML before writing. |
| FR-6  | MUST     | Committer creates a conventional-commit scoped to the touched file's directory. |
| FR-7  | MUST     | Agent re-runs the pipeline after each commit, up to max 3 iterations. |
| FR-8  | MUST     | Agent writes a Markdown transcript with timestamps, prompts, responses, diffs, commits. |
| FR-9  | SHOULD   | Agent supports `PIPELINE_RUNNER=real` to call real `ansible-playbook`. |
| FR-10 | SHOULD   | Agent supports PR-mode (`--require-human-approval`) that opens a PR instead of committing directly. |
| FR-11 | COULD    | Agent handles `ssh_conn_refused` failures with retry/wait_for logic. |
| FR-12 | COULD    | Agent posts a summary to Slack on completion (success or give-up). |
| NFR-1 | MUST     | End-to-end MTTR for a single healed failure ≤ 60 seconds on commodity hardware. |
| NFR-2 | MUST     | Agent never writes outside `ANSIBLE_HEAL_ALLOWED_PATHS` globs (default: `ansible/**`). |
| NFR-3 | MUST     | Agent never force-pushes, never rewrites history, never touches main without `--require-human-approval` in PR-mode. |
| NFR-4 | MUST     | Agent is idempotent: re-running `heal()` on an already-green pipeline is a no-op. |
| NFR-5 | SHOULD   | Agent emits OpenTelemetry spans for each component. |
| NFR-6 | WON'T    | Agent does not perform multi-region failover or capacity planning. |

Each MUST requirement is covered by at least one unit test in `tests/test_agent.py`
and exercised end-to-end by `demo.py`.
...
For production rollout, two additional controls are planned:

- **PR-mode (`--require-human-approval`):** instead of committing directly, the
  agent opens a pull request and waits for human approval before re-running the
  pipeline. Appropriate for shared production playbooks.
- **Deployment-key scope:** the CI container in which the agent runs should have
  a deploy key scoped to a single IaC repo, with read-only SSH keys, and no
  access to production secrets. The agent never needs production secrets — it
  only edits YAML and runs git.
...
| KPI                              | Target (v0.1)                          | Instrumentation                  |
|----------------------------------|----------------------------------------|-----------------------------------|
| MTTR for class-N failures         | ≤ 60 seconds end-to-end                | Transcript timestamps             |
| Autonomous heal rate             | ≥ 80% of class-N failures              | Transcript `final_status` field   |
| LLM-originated patch acceptance  | ≥ 70% of patches used (not fallback)   | Transcript `_fallback_reason`      |
| Regression rate                  | ≤ 2% of heals cause a new failure class| Post-heal pipeline re-run log scan|
| Mean iterations to green         | ≤ 2.0 for class-N failures             | `HealResult.iterations` field      |
...
**Fourth risk — LLM-provider dependency.** The agent depends on the `z-ai` CLI
being available, which couples the MVP to a single LLM provider. *Mitigation:*
the fallback diagnoser, which lets the MVP continue to function — at reduced
coverage — when the LLM is unavailable. A future v0.3 will abstract the LLM
provider behind an interface so alternatives (Claude, GPT, local models) can be
swapped in.
...
- **v0.1 (now):** Simulated Ansible runner, three failure scenarios, local
  commits, Markdown transcript. Intended for demo, evaluation, and team
  alignment — not production traffic.
- **v0.2 (4 weeks):** `PIPELINE_RUNNER=real` support — the agent calls real
  `ansible-playbook` from PATH against a localhost inventory with intentionally
  broken plays. GitHub Actions integration: agent triggers the pipeline via
  `gh` CLI and polls the run status.
- **v0.3 (8 weeks):** PR-mode — the agent opens a pull request with its proposed
  patch and waits for human approval before re-running the pipeline. Slack
  notifications on completion (success or give-up).
- **v1.0 (16 weeks):** Production release — multi-region support, audit dashboard
  aggregating transcripts across the fleet, OpenTelemetry instrumentation, formal
  evaluation rubric for new failure classes. First release enabled by default
  for new repos; existing repos opt in via a config file in their IaC tree.
...
- Conventional Commits 1.0.0 specification — conventionalcommits.org
- Ansible core 2.18 release notes — docs.ansible.com
- z-ai-web-dev-sdk documentation — internal
- GLM-4-Plus model card — internal
